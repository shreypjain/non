...
class TestBasicWorkflows:
    ...
    async def test_complex_multi_stage_workflow(self):
        ...
        with patch("nons.utils.providers.create_provider") as mock_provider_factory:
            mock_provider = MockLLMProvider({"default": "Mock processing result"})
            mock_provider_factory.return_value = mock_provider

            result = await network.forward(
                "Complex input data requiring multi-stage analysis"
            )
    ...
    async def test_workflow_with_custom_configurations(self):
        ...
        resilient_layer_config = LayerConfig(
            error_policy=ErrorPolicy.SKIP_AND_CONTINUE, max_retries=2
        )
        ...
        with patch("nons.utils.providers.create_provider") as mock_provider_factory:
            ...
            result = await network.forward(
                "Input requiring different processing speeds"
            )
...
class TestErrorHandlingWorkflows:
    ...
    async def test_workflow_with_retry_recovery(self):
        ...
        network = NoN.from_operators(
            ["generate", "validate"], layer_config=layer_config
        )
        ...
        with patch("nons.utils.providers.create_provider") as mock_provider_factory:
            ...
            async def failing_then_succeeding(prompt, config):
                ...
                return f"Success on attempt {attempt_count}", self._create_metrics(
                    config
                )
    ...
    async def test_full_workflow_with_tracing(self):
        ...
        with patch("nons.utils.providers.create_provider") as mock_provider_factory:
            ...
            execution_context = ExecutionContext(
                trace_id="test-trace-123", user_id="test-user"
            )

            result = await network.forward(
                "Test input for tracing", execution_context=execution_context
            )
            ...
            trace_spans = [
                ...
                if span["trace_id"] == "test-trace-123"
            ]
    ...
    async def test_workflow_with_rate_limiting(self):
        ...
        rate_limits = {
            ModelProvider.MOCK: RateLimitConfig(
                requests_per_second=2, max_concurrent=2  # Slow rate for testing
            )
        }

        configure_scheduler(
            rate_limits=rate_limits, queue_strategy=QueueStrategy.PRIORITY
        )
        ...
        try:
            ...
            with patch("nons.utils.providers.create_provider") as mock_provider_factory:
                ...
                async def priority_tracking_provider(prompt, config):
                    ...
                    return f"Response from {config.model_name}", self._create_metrics(
                        config
                    )
...
class TestMultiProviderWorkflows:
    ...
    async def test_workflow_with_different_providers(self):
        ...
        openai_config = ModelConfig(
            provider=ModelProvider.OPENAI, model_name="gpt-4", temperature=0.1
        )
...
class TestRealWorldScenarios:
    ...
    async def test_data_processing_pipeline(self):
        ...
        base_analyzer = Node(
            "generate", ModelConfig(provider=ModelProvider.MOCK, model_name="analyzer")
        )
...
class TestPerformanceCharacteristics:
    ...
    async def test_high_concurrency_workflow(self):
        ...
        network = NoN.from_operators(
            ["transform", base_node * 10, "synthesize"]  # 10 parallel operations
        )
    ...
    async def test_memory_efficient_workflow(self):
        ...
        with patch("nons.utils.providers.create_provider") as mock_provider_factory:
            mock_provider = MockLLMProvider({"default": "Memory efficient response"})
            mock_provider_factory.return_value = mock_provider
